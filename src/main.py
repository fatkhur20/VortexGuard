...
from src.config import TELEGRAM_BOT_TOKEN, RULES_TEXT, SLOW_MODE_DURATION, MUTE_DURATION_MINUTES
...
async def handle_new_member(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # We only care if a user *joined* (status member or restricted)
    # Note: Status transitions are complex. Simplification for this task:
    # If status becomes MEMBER, treat as join.
    ...
    if new_member.status in [ChatMemberStatus.MEMBER, ChatMemberStatus.RESTRICTED] and \
       result.old_chat_member.status in [ChatMemberStatus.LEFT, ChatMemberStatus.KICKED]:
        ...
        if anti_raid.register_join():
            ...
            await context.bot.send_message(chat_id, "⚠️ High join rate detected. Anti-raid mode engaged.")
            # Enable slow mode (wait 30s between messages) logic would go here if supported by library easily
            # await context.bot.set_chat_permissions(...) or set_chat_custom_title etc
        ...
        await context.bot.send_message(
            chat_id,
            f"Welcome {user.mention_html()}! Please verify to speak.",
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not member_lifecycle.is_verified(user.id):
        ...
        # For now, assume if they can speak, they might have bypassed or bot failed.
        # But we will check content anyway.
        pass
